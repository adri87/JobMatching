...
$(document).ready(function () {
    ...
    AppViewModel = function () {
 	...
 	self.availableLogos = ko.observableArray(["chico", "chica", "boy", "girl"]);
            ...
            $.ajax({
			    ...
			    url: endPointSemanticMatcher+'{"episteme.search.new_search":' + JSON.stringify([SearchJson]) + '}',
                            ...
                            success: (function (allData) {
                                ...
                                self.reload();
				// Llamada al recomendador de Mahout
				$.ajax({
					type: 'get',
			    		url: endPointMahoutMatcher+'1',
                            		dataType: 'json',
                            		ContentType: 'text/html; charset=UTF-8',
                           		success: (function (allData) {
                                		data = JSON.stringify(allData);
						console.log("data "+data);
						var mahoutJson = JSON.parse(data);
						console.log("mahoutJson "+mahoutJson);
						var parsedJSONMah = JSON.parse(jsonEmpresas);
						console.log("parsedJSONMah "+parsedJSONMah);

						for(var i=0; i < parsedJSONMah.length; i++){
                                			var uri = parsedJSONMah[i]["lmf.uri"];
                                			for(var j = 0; j < mahoutJson.length; j++){
                                				if(mahoutJson[j]["url"] == uri){
                                					parsedJSONMah[i]["weight"] = mahoutJson[j]["sim"];
                                					break;
                                				}
                                			}
	                                	}

                                		ko.mapping.fromJS(parsedJSONMah, self.recommendedData);
                                		self.recommendedData.sortByNumberAsc('weight');
                                		$(".dragContainer").hide().fadeIn();
                               			 self.reload();
					})
				});
				// Fin de la llamada al recomendador de Mahout
                            })
                        });
                    } else {
                        for(var i=0; i < parsedJSON.length; i++){
			...
			$.ajax({
				...
				url: endPointMahoutMatcher+'1',
					...
					for(var i=0; i < parsedJSONMah.length; i++){
                        		...
                        		ko.mapping.fromJS(parsedJSONMah, self.companiesData);
                               		ko.mapping.fromJS(parsedJSONMah, self.recommendedData);
                               		self.recommendedData.sortByNumberAsc('weight');
                        		$(".dragContainer").hide().fadeIn();
                        		self.reload();
		...
		$.ajax({
                    type: "post",
                    url: endPointMahoutMatcher + username + "&offer=" + data + "&action=add",
                    contentType: "text/plain; charset=utf-8",
                    success: function (data) {
                        console.log("Valoracion registrada");
                    },
                    error: function () {
		...
		var username = getCookie("epistemeuser");
                deleteOfferModal(function (result) {
                    if (result) {
           		 $.getJSON(endPointJSON + "episteme.search."+username+"." + item.id(), function (searchData) {
                		var searchDatos = searchData['episteme.search.'+username+"." + item.id()];
                		var data = JSON.parse(searchDatos);
				$.ajax({
                   			type: "post",
                    			url: endPointMahoutMatcher + username + "&offer=" + JSON.stringify(data) + "&action=delete",
                    			contentType: "text/plain; charset=utf-8",
                    			success: function (data) {
                        			console.log("Valoracion eliminada");
                    			},
                    			error: function () {
                        			//alert("Error al guardar");
                    			}
